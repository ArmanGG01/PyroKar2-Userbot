...
@Client.on_message(filters.command(["playlist"], ".") & filters.me
)
...
@Client.on_message(filters.command(["end"], ".") & filters.me
)
async def leave_vc_test(client, message):
    ...
    if os.path.exists(group_call.input_filename):
        os.remove(group_call.input_filename)
    await group_call.stop()
    await edit_or_reply(message, f"`❌Gua Turun Dulu Ya Kontol : {message.chat.title} - Vc`")

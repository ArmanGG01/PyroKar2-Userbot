...
from PyroKar import *
from PyroKar.helper.adminHelpers import DEVS, WHITELIST
from PyroKar.helper.basic import edit_or_reply
from PyroKar.helper.PyroHelpers import get_ub_chats
from PyroKar.utils import extract_user, extract_user_and_reason
...
async def gban_user(bot: Client, message: Message):
...
async def ungban_user(bot: Client, message: Message):
    user_id, reason = await extract_user_and_reason(message, sender_chat=True)
    if message.from_user.id != client.me.id:
...
async def gbanlist(bot: Client, message: Message):
...
async def gmute_user(bot: Client, message: Message):
...
async def ungmute_user(bot: Client, message: Message):
    args = await extract_user(message)
    reply = message.reply_to_message
    Man = await edit_or_reply(message, "`Processing...`")
    if args:
...
async def gmutelist(bot: Client, message: Message):
...
async def globals_check(bot: Client, message: Message):
    if not message:
        return
